...
from isotp.lin_message import LinMessage
...
class PDU:
    """
    Converts a Lin Message into a meaningful PDU for the IsoTP protocol. This is used internally by the LinStack to process received messages and prepare messages to send. It is not intended to be used directly by the user.
    ...
    def __init__(self, msg: LinMessage, start_of_data: int = 0) -> None:
...
class TransportLayerLogic:
    ...
    class Params:
        ...
        def validate(self) -> None:
            ...
            if self.tx_data_length > 8 or self.tx_data_length < 2:
                raise ValueError('tx_data_length must in the range of 2 to 8 bytes for LIN communication')
            ...
            if self.tx_data_min_length is not None:
                ...
                if self.tx_data_min_length > 8 or self.tx_data_min_length < 2:
                    raise ValueError('tx_data_min_length must in the range of 2 to 8 bytes for LIN communication')
    ...
    RxFn = Callable[[float], Optional[LinMessage]]
    TxFn = Callable[[LinMessage], None]
    ...
    tx_standby_msg: Optional[LinMessage]
    rx_state: RxState
    tx_state: TxState
    ...
    # in some cases like lin communication, there is not concept of flow control, so we need to disable the request of flow control in reception process and directly switch to WAIT_CF state. This flag is used for this purpose
    ...
    def _process_rx(self, msg: LinMessage) -> ProcessRxReport:
        """Process the reception of a LIN message. Moves the reception state machine accordingly and optionally"""
            ...
            if pdu.type == PDU.Type.SINGLE_FRAME:
                ...
                if pdu.seqnum == expected_seqnum:
                    ...
                    if len(self.rx_buffer) >= self.rx_frame_length:
                        ...
                        # if self.params.blocksize > 0 and (self.rx_block_counter % self.params.blocksize) == 0 and self.is_need_fc:
    ...
    def _process_tx(self) -> ProcessTxReport:
        ...
        #         # self.tx_state = self.TxState.WAIT_FC  # Just to be sure
        #     else:
            ...
            if self.tx_standby_msg is not None:
                if len(self.tx_standby_msg.data) <= allowed_bytes:
                    ...
                    if self.tx_state == self.TxState.TRANSMIT_FF_STANDBY:
                        ...
                        # else:
                            self.tx_state = self.TxState.TRANSMIT_CF  # If we don't need flow control, we can directly start sending consecutive frames
                    else:
    ...
    #     )
    def _make_tx_msg(self, arbitration_id: int, data: bytes) -> LinMessage:
        data = self._pad_message_data(data)
        return LinMessage(
            arbitration_id=arbitration_id,
            dlc=self._get_dlc(data, validate_tx=True),
            pid=self.params.protect_id,
            data=data,
        )
    ...
    def _get_lin_size(self, size: int) -> int:
        if size <= 8:
            return size
        raise ValueError("Impossible data size for LIN : %d " % (size))
    
    def _get_nearest_can_fd_size(self, size: int) -> int:
        self._get_lin_size(size)
    ...
    def _start_reception_after_first_frame_if_valid(self, pdu: PDU, ) -> bool:
        ...
        if pdu.rx_dl < 2 or pdu.rx_dl > self.params.rx_data_length:
...
class TransportLayer(TransportLayerLogic):
    ...
    rx_relay_queue: queue.Queue[Optional[LinMessage]]
    ...
    def __init__(self,
        ...
        TransportLayerLogic.__init__(self, rxfn, txfn, address, error_handler, params, post_send_callback, is_need_fc)

    # def _read_relay_queue(self, timeout: Optional[float]) -> Optional[CanMessage]:
    def _read_relay_queue(self, timeout: Optional[float]) -> Optional[LinMessage]:
        try:
...
def python_lin_tx_linbus_3plus(owner: BusOwner, msg: LinMessage) -> None:
    owner.bus.send(lin.LinMessage(id=msg.id, data=msg.data,
                                  pid=msg.pid))  # type:ignore
...
def python_lin_tx_linbus_3minus(owner: BusOwner, msg: LinMessage) -> None:
    owner.bus.send(lin.LinMessage(id=msg.id, data=msg.data, pid=msg.pid))  # type:ignore
...
def _make_python_lin_tx_func(owner: BusOwner) -> Callable[[LinMessage], None]:
    message_input_args = inspect.signature(lin.LinMessage.__init__).parameters
    if 'pid' in message_input_args:
        return functools.partial(python_lin_tx_linbus_3plus, owner)
    else:
        return functools.partial(python_lin_tx_linbus_3minus, owner)
...
def _python_lin_to_isotp_message(msg: Optional["lin.LinMessage"]) -> Optional[LinMessage]:
    if msg is None:
        return None

    return LinMessage(id=msg.id, data=msg.data, is_response=msg.is_response)

class LinStack(TransportLayer):
    bus: lin.linBusABC
    
    def _rx_linbus(self, timeout: float) -> Optional[LinMessage]:
        msg = self.bus.recv(timeout)
        if msg is None:
            return None
        return _python_lin_to_isotp_message(msg)
    
    def __init__(self, bus: "lin.linBusABC", *args: Any, **kwargs: Any):
        if not _lin_available:
            raise RuntimeError(f"python-lin is not installed in this environment and is required for the {self.__class__.__name__} object.")
        
        self.set_bus(bus)
        kwargs.update(dict(
            rxfn=self._rx_linbus,
            txfn=_make_python_lin_tx_func(self),
        ))
        super().__init__(*args, **kwargs, is_need_fc=False)  # LIN does not have flow control
        
    def set_bus(self, bus: "lin.linBusABC") -> None:
        if not isinstance(bus, lin.linBusABC):
            raise ValueError('bus must be a python-lin linBusABC object')
        self.bus = bus
