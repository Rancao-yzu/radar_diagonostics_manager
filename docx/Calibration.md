...
自01后开始，按顺序解析
### 标定参数表
> 大端序，7 个 int32 紧密打包（`struct.Struct('>iiiiiii')`）。所有字段下发时按缩放因子 ×N 后取整为 int32。
| 占用字节数 | 字段名称 | 英文标识 | 数据类型 | 物理单位 | 缩放因子 | 说明 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 4 | 可通行高度(车辆高度) | vehicle_height | int32 | m | ×1000 | 可通行高度(车辆高度)，单位米 |
| 4 | 雷达纵向安装偏差 | radar_x_offset | int32 | m | ×1000 | 雷达相对车辆后轴中心的纵向偏差，向前为正 |
| 4 | 雷达横向安装偏差 | radar_y_offset | int32 | m | ×1000 | 雷达相对车辆后轴中心的横向偏差，左正右负 |
| 4 | 雷达对地安装高度 | radar_z_offset | int32 | m | ×1000 | 雷达对地安装高度，向上为正 |
| 4 | 雷达水平偏转角 | radar_yaw_angle | int32 | ° | ×10000 | 雷达相对主车坐标系的水平偏转角，水平向右为正 |
| 4 | 雷达俯仰角 | radar_pitch_angle | int32 | ° | ×10000 | 标定完成后雷达相对主车坐标系的俯仰角，竖直向上偏转为正 |
| 4 | 雷达横滚角 | radar_roll_angle | int32 | ° | ×10000 | 雷达相对主车坐标系的横滚角，绕x轴顺时针为正 |
| 35 | 保留 | reserve | - | - | - | 全 0x00 填充 |

Data=01 + 标定参数表(28B) + 保留(35B) = 64字节
